...
module.exports = =>
  page = 0
  while page++ < 48
    url = "http://wap.sciencenet.cn/blog.php?mod=recommend&page=#{page}&op=4&inajax=1&mobile=1"
    ex = await req.ex(url)
    for i in ex.li('href="{}"')
      url = "http://wap.sciencenet.cn/"+i
      ex = await req.ex(url,{responseType: 'arraybuffer'})
      ex.html = iconv.decode(ex.html,'GBK')
      console.log ex.one('<div class="blogCon">{}<p style="font-size:13px; color:#850f0f">转载本文')
